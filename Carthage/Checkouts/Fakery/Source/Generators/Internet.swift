...
public class Internet: Generator {
  ...
  public func username(separator: String? = nil) -> String {
    ...
    let text = components[0..<count].joined(separator: gap)
    return text.replacingOccurrences(of: "'", with: "", options: .diacriticInsensitive,  range: text.range(of: text)!).lowercased()
  }
